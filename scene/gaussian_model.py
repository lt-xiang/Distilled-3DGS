...
class GaussianModel:
    ...
    def add_noises(self, grad_threshold, extent, min_scale=1.0, ratio=0.1):
        
        grads = self.xyz_gradient_accum / self.denom
        grads[grads.isnan()] = 0.0
        
        reset_masks = torch.where(torch.norm(grads, dim=-1) >= grad_threshold, True, False)
        reset_masks = torch.logical_and(reset_masks,
                                              torch.max(self.get_scaling, dim=1).values <= self.percent_dense * extent)
        
        print(f"noise num: {reset_masks.sum()} / {len(self._xyz)}")
    ...
    def reinitial_pts(self, pts, rgb):
        ...
        print(f"re-initialize point size: {fused_point_cloud.shape[0]}" )
